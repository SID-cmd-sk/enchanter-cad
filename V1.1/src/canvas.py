...
class Canvas(QWidget):
    ...
    def __init__(self, drawing, parent=None):
        ...
        # SolidWorks-style live constraint inference
        self._infer_glyph = None    # (glyph, world_point) currently inferred
        self._infer_factory = None  # constraint factory for current inferred relation
        self._infer_moving = None   # entity being created/edited (inference target)
    ...
    def _snap_point(self, wx, wy):
        ...
        if "NEA" in self.snap_modes:
            ...
            if near is not None:
                ...
                return near
        # ---- SolidWorks-style live constraint inference ----
        self._infer_glyph = None
        self._infer_factory = None
        if self.active_gen and self._pending_kind == Prompt.POINT:
            try:
                from constraints import infer_point_constraint
                res = infer_point_constraint(wx, wy, self.drawing.entities,
                                             moving_ent=self._infer_moving,
                                             tol=10.0 / self.scale)
                if res:
                    glyph, snap, factory = res
                    self._infer_glyph = (glyph, snap)
                    self._infer_factory = factory
                    self._snap_marker = (snap[0], snap[1], glyph)
                    return snap
            except Exception:
                pass
        return (wx, wy)
    ...
    def paintEvent(self, ev):
        ...
        self._draw_infer_glyph(p)
        p.end()

    def _draw_infer_glyph(self, p):
        """Draw the SolidWorks-style relation glyph at the inferred snap point
        so the user sees which constraint will be applied before clicking."""
        if not self._infer_glyph:
            return
        glyph, (gx, gy) = self._infer_glyph
        sp = self.world_to_screen(gx, gy)
        s = 9
        ...
        cx, cy = sp.x(), sp.y()
        if glyph == "COINCIDENT":
            p.drawLine(int(cx - s * 0.5), int(cy), int(cx + s * 0.5), int(cy))
            p.drawLine(int(cx), int(cy - s * 0.5), int(cx), int(cy + s * 0.5))
        elif glyph == "TANGENT":
            p.drawArc(int(cx - s * 0.6), int(cy - s * 0.6), int(s * 1.2), int(s * 1.2), 0, 16 * 360)
        elif glyph == "PERPENDICULAR":
            p.drawLine(int(cx - s * 0.5), int(cy + s * 0.3), int(cx + s * 0.5), int(cy + s * 0.3))
            p.drawLine(int(cx + s * 0.1), int(cy + s * 0.5), int(cx + s * 0.1), int(cy - s * 0.5))
        elif glyph == "PARALLEL":
            for off in (-3, 3):
                p.drawLine(int(cx - s * 0.5), int(cy + off), int(cx + s * 0.5), int(cy + off))
        elif glyph in ("HORIZONTAL", "VERTICAL"):
            if glyph == "HORIZONTAL":
                p.drawLine(int(cx - s * 0.6), int(cy), int(cx + s * 0.6), int(cy))
            else:
                p.drawLine(int(cx), int(cy - s * 0.6), int(cx), int(cy + s * 0.6))
        # label
        p.setPen(QPen(col, 1))
        p.drawText(int(sp.x()) + s + 2, int(sp.y()) + 4, glyph)
    ...
    def mousePressEvent(self, ev):
        ...
        if ev.button() == Qt.MouseButton.LeftButton:
            if getattr(self, "_manual_cb", None) is not None:
                hit = self._hit_test(wx, wy)
                if hit:
                    self._manual_picks.append(hit)
                    self._manual_cb([hit])
                    self._manual_cb = None
                return
            if self.active_gen and self._pending_kind == Prompt.POINT:
                pt = self._snap_point(wx, wy)
                # create the inferred geometric constraint (SolidWorks-style):
                # the relation the user "hovered into" is applied automatically
                if self._infer_factory is not None and self._infer_moving is not None:
                    try:
                        con = self._infer_factory()
                        if con is not None:
                            self.drawing.add_constraint(con)
                            self.drawing.solve_constraints()
                            pt = self._infer_glyph[1] if self._infer_glyph else pt
                    except Exception:
                        pass
                    self._infer_factory = None
                    self._infer_glyph = None
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self._grip_drag:
            ...
            # SolidWorks-style: while dragging a vertex, infer a relation to
            # nearby geometry and snap + show a glyph (e.g. coincident to a
            # corner, tangent to a circle, perpendicular to a line).
            self._infer_glyph = None
            self._infer_factory = None
            try:
                from constraints import infer_point_constraint
                ...
                ent = g["ent"]
                which = g["ref"]
                if ent.kind in ("LINE",) or (ent.kind == "LWPOLYLINE"):
                    res = infer_point_constraint(wx, wy, self.drawing.entities,
                                                 moving_ent=ent, tol=10.0 / self.scale)
                    if res:
                        glyph, snap, factory = res
                        self._apply_grip({"ent": ent, "ref": which, "pt": (snap[0], snap[1])}, snap[0], snap[1])
                        self._infer_glyph = (glyph, snap)
                        self._infer_factory = factory
            except Exception:
                pass
            self._solve_constraints_live()
            self.update()
            return
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if ev.button() == Qt.MouseButton.LeftButton and self._grip_drag:
            if self._infer_factory is not None:
                try:
                    con = self._infer_factory()
                    if con is not None:
                        # The drag's geometry change was already snapshotted at
                        # mousedown; merge the inferred constraint into the same
                        # undo step instead of pushing a second history entry.
                        self.drawing.add_constraint(con)
                        self.drawing.solve_constraints()
                except Exception:
                    pass
    ...
    def _solve_constraints_live(self):
        """Re-run the parametric solver after an interactive edit so all
        constrained geometry snaps into place (SolidWorks-style)."""
        if self.drawing.constraints:
            try:
                self.drawing.solve_constraints()
            except Exception:
                pass

    def begin_manual_constraint(self, callback):
        """Enter a temporary mode where left-clicks pick entities (for the
        Constraint Manager's manual-add flow) and forward them to `callback`."""
        self._manual_cb = callback
        self._manual_picks = []

    def _end_manual_constraint(self):
        self._manual_cb = None
        self._manual_picks = []
