...
class IconFactory:
    ...
    def _text_general(self, p, s, m, n):
        # filled "A" block - the original CAD font look
        p.setFont(QFont("Arial", int(s * 0.5), QFont.Weight.Bold))
        p.drawText(QRect(0, 0, s, s), Qt.AlignmentFlag.AlignCenter, "A")

    def _text_single(self, p, s, m, n):
        # one open centerline stroke (engrave)
        p.drawLine(m, s - m, s - m, m)
        p.drawLine(int(s * 0.5), m, int(s * 0.5), s - m)
    ...
    def _manager(self, p, s, m, n):
        p.drawRect(m, m, s - 2 * m, s - 2 * m)
        p.drawLine(m, int(s * 0.4), s - m, int(s * 0.4))
        p.drawLine(m, int(s * 0.62), s - m, int(s * 0.62))
        p.drawRect(int(s * 0.5), int(s * 0.52), int(s * 0.28), int(s * 0.28))

    def _smart(self, p, s, m, n):
        # a dimension arrow with an implied "auto" tag
        p.drawLine(m + 2, int(s * 0.7), s - m - 2, int(s * 0.7))
        p.drawLine(m + 2, int(s * 0.6), m + 2, int(s * 0.8))
        p.drawLine(s - m - 2, int(s * 0.6), s - m - 2, int(s * 0.8))
        p.drawLine(int(s * 0.45), int(s * 0.7), int(s * 0.55), int(s * 0.45))
        p.drawLine(int(s * 0.55), int(s * 0.45), int(s * 0.68), int(s * 0.5))
    ...
    _glyphs = {
        ...
        "LAYER": _layer, "UNDO": _copy, "REDO": _move, "MANAGER": _manager,
        "SMARTDIM": _smart,
    }
...
class Ribbon(QTabWidget):
    ...
    def _build(self):
        ...
        self._tab("Annotate", [
            ("Dimension", [("SMARTDIM", "Smart"), ("DIMLINEAR", "Dim"), ("DIMRAD", "Rad"),
                           ("DIMDIA", "Dia"), ("DIMANG", "Ang"), ("LEADER", "Lead")]),
            ("Text", [("TEXT", "Text"), ("TEXTS", "1-Line"),
                       ("MTEXT", "MText")]),
        ])
        self._tab("View", [
        ...
        self._tab("Parametric", [
            ("Constraints", [("AUTOCONSTRAIN", "Auto"), ("CONSTRAIN", "Add"),
                             ("MANAGER", "Manager"), ("REBUILD", "Rebuild"),
                             ("DELCONSTRAINT", "Del")]),
