...
def cmd_line(ctx):
    ...
    while True:
        ...
        if p2 is None:
            break
        ctx.drawing.add(Line(p1, p2, layer=ctx.drawing.current_layer))
        p1 = p2
...
def cmd_circle(ctx):
    ...
    if r < 1e-9:
        ...
        return
    ctx.drawing.add(Circle(c, r, layer=ctx.drawing.current_layer))
...
def cmd_text(ctx):
    ctx.drawing.push_undo()
    p = yield Prompt(Prompt.POINT, "Specify start point:")
    h_in = yield Prompt(Prompt.TEXT, "Specify text height:", default="2.5")
    try:
        h = float(h_in) if h_in else 2.5
    except ValueError:
        h = 2.5
    s = yield Prompt(Prompt.TEXT, "Enter text:")
    ctx.drawing.add(TextEntity(p, h, s or "", layer=ctx.drawing.current_layer))
    yield done("Text created.")
...
def cmd_dim_linear(ctx):
    ...
    ctx.drawing.push_undo()
    ctx.drawing.add(dim)
    yield done(f"Linear dimension: {dim.measurement:.2f}")
...
def cmd_dim_radius(ctx):
    ...
    ctx.drawing.push_undo()
    ctx.drawing.add(dim)
    yield done(f"Radius dimension: R{dim.measurement:.2f}")
...
def cmd_dim_diameter(ctx):
    ...
    ctx.drawing.push_undo()
    ctx.drawing.add(dim)
    yield done(f"Diameter dimension: {dim.measurement:.2f}")
...
def cmd_dim_angular(ctx):
    ...
    ctx.drawing.push_undo()
    ctx.drawing.add(dim)
    yield done(f"Angular dimension: {dim.measurement:.1f} deg")
