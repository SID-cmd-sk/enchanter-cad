...
def _angular_sector(l1, l2, loc):
    ...
    if V is None:
        ...
        return V, s, e, ang, R
    # ray directions of both infinite lines (use line direction, robust even
    # when the vertex coincides with an endpoint)
    rays = []
    for ln in (l1, l2):
        d = _unit((ln.p2[0] - ln.p1[0], ln.p2[1] - ln.p1[1]))
        a0 = math.atan2(d[1], d[0])
        rays.append(a0)
        rays.append(a0 + math.pi)
    uniq = []
    for r in sorted(rays):
        if not uniq or abs(r - uniq[-1]) > 1e-9:
            uniq.append(r)
    ...
    for i in range(n):
        s = uniq[i]
        e = uniq[(i + 1) % n] + (2 * math.pi if (i + 1) == n else 0)
        inside = (s <= ang_loc <= e) or (i == n - 1 and (ang_loc >= s or ang_loc < uniq[0]))
        if inside:
            span = e - s
            if best is None or span < best[0]:
                best = (span, s, e)
    if best is None:
        best = (math.pi, 0.0, math.pi)
    span, s, e = best
